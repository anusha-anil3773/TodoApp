...
const getList=async ()=>{
    try{
        ...
        lists.forEach((el,index)=>{
            listcontent+=`<li class="list-group-item ${el.completed?'disabledList':''} ${index%2?'list-group-item-light':'list-group-item-light'}"> <input type="checkbox" class="checkbox" ${el.completed?' checked':''}/> <label for=""> ${el.title}</label></li>`
        });
...
const promiseCall=()=>{
    alertPromise().then((data)=>{
        alert(`Done ${data} activities today.... Congrats!`);
    })
    .catch((err)=>{
